...
public class BufferedFileInputStream extends FileInputStream {
  ...
  int size; // 배열에 저장되어 있는 바이트의 수 (실제 몇 개의 값을 읽었는지)
  int cursor; // 앞으로 읽을 바이트의 인덱스(배열의 위치)
  ...
  public int read() throws IOException {
    ...
    // 바이트 배열에 있는 값을 그냥 리턴해서는 안된다!
    return buf[cursor++] & 0x000000ff; // & => 둘 다 1이거나 0이어야 1 ?

    // 위의 리턴 문장은 컴파일할 때 아래의 문장으로 바뀐다.
    //    int temp;
    //    temp = buf[cursor];
    //    cursor++;
