...
- 각각의 문자는 규칙에 따라 0 ~ 65535까지의 값으로 메모리에 저장된다.
...
- UCS의 코드 값 중에서 00 ~ 7F(127개)까지 ASCII에 해당하는 UCS 코드는 그대로 1바이트로 표현한다.
  따라서 ASCII를 기본으로 사용하는 시스템의 경우 UTF-8로 인코딩 된 데이터를 특별한 처리없이 
  그대로 읽고 쓸 수 있다.
 ...
 * 참고!
- 윈도우 OS에서는 LFCR 2바이트를 사용하여 줄 바꿈을 표시한다.
  LF = Line Feed, CR = Carriage Return (0D0A)
...
- 가장 널리 쓰이며 대부분의 서유럽 언어를 지원한다. 
  ...
  프랑스어, 핀란드어, 동유럽의 알바니아어, 아프라카의 스와힐리어, 아프리칸스어 등
...
5) MS949 (=CP949; 2바이트)
- 윈도우 OS의 인코딩 규칙이다.
- EUC-KR + 현재 유통되는 한글 음절 추가 = MS949 (11,172자 + 한국 전용 한자 + 옛한글 등)
